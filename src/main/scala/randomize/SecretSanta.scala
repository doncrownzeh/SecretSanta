...
object SecretSanta {
  def apply(participants: List[Participant]): List[ParticipantsPairing] = {
    val givers = participants
    val receivers = Random.shuffle(participants)

    @tailrec
    def randomize(givers: List[Participant],
                  receivers: List[Participant],
                  pairings: List[ParticipantsPairing]): List[ParticipantsPairing] = {
      givers match {
        ...
        case firstGiver :: secondGiver :: Nil =>
          if (firstGiver != receivers.head && secondGiver != receivers.tail.head)
            ...
            pairings :+ ParticipantsPairing(firstGiver, receivers.tail.head) :+ ParticipantsPairing(secondGiver, receivers.head)
        case head :: tail =>
          val receiver = receivers.filter(x => x != head).head
